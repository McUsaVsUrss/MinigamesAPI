...
import com.comze_instancelabs.minigamesapi.events.*;
...
public class Arena {
    ...
    public HashMap<String, Long> lastdamageTime = new HashMap<String, Long>();
     ...
     *
     * @param plugin JavaPlugin the arena belongs to
     ...
     */
    public void reset() {
        Cuboid cuboid = getBoundaries();
        int x_low = cuboid.getLowLoc().getBlockX();
        int z_low = cuboid.getLowLoc().getBlockZ();
        int x_high = cuboid.getHighLoc().getBlockX();
        int z_high = cuboid.getHighLoc().getBlockZ();
        int ki = 0;
        for(int x = x_low/16; x<x_high/16; x++){
            for(int z = z_low/16; z<z_high/16; z++){
                Chunk chunk = cuboid.getWorld().getChunkAt(x,z);
                chunk.load();
        ...
        ArenaLogger.debug("Removing " + ki + " mobs or entities.");

        ArenaLogger.debug("Resetting using new method.");
        try {
            Util.loadArenaFromFileSYNC(plugin, this);
        } catch (Exception e) {
            ArenaLogger.debug("Error resetting map using new method. " + e.getMessage());
        }
    }
     ...
     *
     * @param playername The player that got eliminated
     */
    public void onEliminated(String playername) {
        if (lastdamager.containsKey(playername)) {
            ...
            lastdamageTime.remove(playername);
        } else {
            pli.getStatsInstance().addDeath(playername);
        }
    }
