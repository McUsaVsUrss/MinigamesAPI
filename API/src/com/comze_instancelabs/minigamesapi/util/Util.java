...
import com.commander.nolag.api.Utils;
...
public class Util {
    ...
    public static Sign getSignFromArena(JavaPlugin plugin, String arena) {
        ...
        if (!pli.getArenasConfig().getConfig().isSet("arenas." + arena + ".sign.world")) {
            ArenaLogger.debug("FAILED UPDATING SIGN! Cannot find world");
            return null;
        }
        World world = Bukkit.getServer().getWorld(pli.getArenasConfig().getConfig().getString("arenas." + arena + ".sign.world"));
        if (world != null) {
            Location loca = new Location(world, pli.getArenasConfig().getConfig().getInt("arenas." + arena + ".sign.loc.x"), pli.getArenasConfig().getConfig().getInt("arenas." + arena + ".sign.loc.y"), pli.getArenasConfig().getConfig().getInt("arenas." + arena + ".sign.loc.z"));
            BlockState s = loca.getBlock().getState();
            if (s == null) {
                ArenaLogger.debug("FAILED UPDATING SIGN! Block state = null");
                return null;
            }
            ...
            if (s instanceof Sign) {
                ...
                ArenaLogger.debug("FAILED UPDATING SIGN! Block state = " + s.getClass());
            }
            return s_;
        } else {
            ArenaLogger.debug("FAILED UPDATING SIGN World is null");
        }
        return null;
		/*Location b_ = new Location(, );
		if (b_ != null) {
			if (b_.getWorld() != null) {
    ...
    public static void updateSign(JavaPlugin plugin, Arena arena) {
        ...
        if (s != null) {
            ...
            ArenaLogger.debug("FAILED Updating sign for arena " + arena.getInternalName() + " in " + plugin.getName() + "! Fatal error. Report to Carra");
        }
        try {
            if (plugin.isEnabled()) {
