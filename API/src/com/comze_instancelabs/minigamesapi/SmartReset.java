...
/*		int rolledBack = 0;
		...
		if (changed.size() != 0) {
			ArenaLogger.debug("Changed block isn't empty, Carra thinks thats it's a good thing to ignore it! " + changed.size());
		...
		ArenaLogger.debug(failedblocks.size() + " failed blocks to redo.");
...
/*		time = System.currentTimeMillis();
		ArenaLogger.debug(changed.size() + " to reset for arena " + a.getInternalName() + ".");
